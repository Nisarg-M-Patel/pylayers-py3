...
sd = 1.*np.ones(Nr)
plt.ion()
Ds,Dh,D1,D2,D3,D4 = diff(fGHz,phi0,phi,si,sd,N,mat0,matN,mode='tab',debug=True)
# Ds,Dh,D1,D2,D3,D4 = diff(fGHz,phi0,phi,si,sd,N,mat0,matN,mode='tab',debug=True)
...
plt.plot(phi*180/np.pi,np.abs(Ed4[0,:]),'r',label='RSBinf')
plt.plot(phi*180/np.pi,np.abs(Ers[0,:]),'r')
plt.plot(phi*180/np.pi,np.abs(Ec4[0,:]),'r')
plt.legend()
plt.savefig('test_diffraction_1.png')
...
plt.legend()
plt.title('$\\alpha=4^{\circ},\phi_0=55^{\circ},f= 3 GHz, sd=1m$',fontsize=14)
plt.grid()
plt.savefig('test_diffractionRT_2.png')
