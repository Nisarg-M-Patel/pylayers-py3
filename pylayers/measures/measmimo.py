...
if os.path.isfile(filecalh5):
    C = Mesh5(filecalh5)
    if len(list(C.gcal.keys()))>1:
        print(" The Full MIMO calibration file contains the following group of calibration (gcal) : ")
        for g in list(C.gcal.keys()):
            print(g.replace('cal',''))
        num = input("Select group of mimo calibration number ")
    ...
    rep  = input("Do you want to create one  ? (Y/N) ")
    if rep=='Y':
        mimocal=True
        gcalm = '1'
    else:
...
if os.path.isfile(filemesh5):
    ...
    sical = input("Select a configuration number for the next measurement series:")
    ical = int(sical)

else:
    print("Warning there is no measurement file in your measurement directory")
    rep  = input("Do you want to make a single calibration (Y/N) ")
    if rep=='Y':
        calibration=True
    else:
        calibration=False
...
if measure:
   ...
   sheight = input('Transmitting antenna height (meters) : ')
