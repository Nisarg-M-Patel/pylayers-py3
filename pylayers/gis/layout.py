...
# Modern replacement for deprecated NetworkX gpickle functions
...
class Layout(pro.PyLayers):
    ...
    def __add__(self, other):
        ...
        if type(other) == np.ndarray:
            ...
            Ls.Gs.nodes.update(other.Gs.nodes(data=True))
    ...
    def offset_index(self, offp=0, offs=0):
        ...
        newpoint = dict((k - offp, v) for k, v in self.Gs.nodes(data=True) if k < 0)
        assert (np.array(list(newpoint.keys())) < 0).all()

        newseg = dict((k + offs, v) for k, v in self.Gs.nodes(data=True) if k > 0)
        ...
        for n in lk:
            if ((n < 0) & (self.Gs.degree(n) == 0)):
                ...
                try:
                    ...
                    pass
        self.Np = len(np.nonzero(np.array(list(self.Gs.nodes())) < 0)[0])
...
#
    ...
    def showGs(self, **kwargs):
        ...
        if kwargs['ndlist'] == []:
            tn = np.array(list(self.Gs.nodes()))
            u = np.nonzero(tn < 0)[0]
            ndlist = tn[u]
    ...
    def buildGt(self, check=True,difftol=0.01,verbose=False,tqdmpos=0):
        ...
        self.dca = {}
        for seg, d in self.Gs.nodes(data=True):
            if seg > 0:
    ...
    def _delaunay(self, poly, polyholes=[]):
        ...
        if len(pucs) != 0:
            ...
            if polyholes == []:
                ...
                cpdiff = poly.difference(unary_union(cpolys))
    ...
    def buildGi(self,verbose=False,tqdmpos=0):
        ...
        if self.typ=='outdoor':
            for k in list(dict(self.Gi.nodes(data=True)).keys()):
                # R and T 
                if len(k)>1:
    ...
    def showG(self, graph='s', **kwargs):
        ...
        if len(kwargs['edgelist']) == 0:
            if kwargs['mode'] == 'cycle':
                for k, ncy in enumerate(list(dict(self.Gt.nodes(data=True)).keys())):
                    if k != 0:
                        ...
                        args['ax'] = ax
            if kwargs['mode'] == 'room':
                for k, nro in enumerate(list(dict(self.Gr.nodes()))):
                    if k != 0:
    ...
    def thwall(self, offx, offy):
        ...
        """
        keyn = list(dict(self.Gs.nodes(data=True)).keys())
        walls = []
        for nd in keyn:
            if nd > 0:
    ...
    def pt2cy(self, pt=np.array((0, 0))):
        ...
        for ncy in list(dict(self.Gt.nodes(data=True)).keys()):
            if ncy > 0:
    ...
    def pt2ro(self, pt=np.array((0, 0))):
        ...
        ptsh = sh.Point(pt[0], pt[1])
        ptshinroom = False
        for nr in list(dict(self.Gr.nodes())):
    ...
    def geomfile(self, centered=False):
        ...
        en = len(np.where(np.array(list(dict(self.Gs.nodes(data=True)).keys())) > 0)[0])
        if en != self.Ns:
...
#
        ...
        ik = 0
        dikn = {}
        for i in list(dict(self.Gs.nodes(data=True)).keys()):
            if i > 0:  # segment
